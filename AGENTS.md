...
- 新建完整 run 时，`run-id` 固定使用 `python bin/generate-run-id.py` 生成；默认格式为 `<YYYYMMDD-HHMMSS>`，OpenCode/TestAgent hook 可通过 `TEST_ANALYSIS_RUN_ID` 注入当前 `sessionID` 作为 run-id。
