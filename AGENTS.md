...
测试分析方案不生成测试设计项。测试设计方案不生成完整测试用例、前置步骤、测试步骤、自动化脚本或可执行测试数据清单。`测试点明细` 的核心是说明测试点下需要评审的规则分支、路径分支、状态分支、权限分支、接口契约分支或风险分支；`测试设计项` 的核心是说明用哪些代表性条件、数据、状态或组合覆盖测试点明细。

主交付件术语与缩写固定为：测试场景 `SC-*`、测试点 `TP-*`、测试点明细 `TP-*-*`、测试设计项 `TDI-*`。分析方案不使用 `TDI-*`；设计方案不使用 `TD-*`、`TC-*`、`TCT-*`、`TI-*`、`ITP-*` 或 `ITDI-*`。主交付件不展开英文全名。
