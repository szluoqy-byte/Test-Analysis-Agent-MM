...
分析阶段内部先冻结 `process/scenario-tree.md`，再按叶子 SC 生成 Markdown TP 切片并在阶段边界固化结果 JSON。SC 冻结后，TP 阶段不得修改 SC。
...
设计阶段内部按每个 TP 生成 Markdown TC 切片，并在阶段边界一次性固化设计结果 JSON。TC 阶段不得修改 SC 或 TP。
...
```text
测试分析方案 + 测试设计方案 + analysis/design fact-coverage-map + analysis/design final-report
```

全流程阶段只做高层编排和阶段交接：优先启动独立 analysis subagent 执行 `test-analysis-workflow`，再把完整 `deliverables/test-analysis-solution.json` 显式传给独立 design subagent 执行 `test-design-workflow`。它不复制分析或设计阶段内部校验、review、fact-coverage-map、coverage 或 final-report 逻辑。
...
analysis/design 阶段之间只通过 run 目录下的 canonical JSON 和固定过程/报告文件交接，不通过聊天上下文、自然语言总结或隐式记忆传递业务事实。analysis subagent 不输出 TC；design subagent 不重新分析或改写 SC/TP。
