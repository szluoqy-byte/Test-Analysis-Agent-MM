...
- 如果规格没有错误码或响应结构，预期结果只写输入可支撑的保守预期，不得编造。
...
- 合法请求创建支付请求；非法字段的错误码、错误提示或状态变化如果规格未说明，预期结果只写输入可支撑的保守拒绝结果。
...
- `TDI-*` 不写“请求失败”“显示错误信息”；错误提示、错误码和状态变化由叶子节点预期结果承接，依据不足时只写输入可支撑的保守预期。
