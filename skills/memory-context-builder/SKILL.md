...
description: 每次需求分析或测试设计开始前使用，用于从 rules、精简 memory、knowledge 和本地配置中选择相关上下文，生成紧凑的本次上下文包，避免长期资料全量注入。
---

# 记忆上下文构建 Skill

本 skill 在每次分析或设计开始时使用，目标是为当前需求挑选适用的强制规则、项目语境、项目测试经验、个人偏好和本地检查关注点。project 与 personal 不是可有可无的“附加资料”，而是当前 run 的一等输入源；即使没有命中正文，也必须记录绑定、扫描、未采用原因、适用 rules 和项目知识阶段绑定。
...
Rules 是高优先级约束源，不是普通 knowledge。适用优先级为：

```text
当前用户明确指令 > rules > 当前输入文档 > memory > knowledge > skill 默认流程
```

本 skill 必须扫描并记录适用于当前 run 的 rules：

| 层级 | 读取规则 | 说明 |
|---|---|---|
| core rules | 读取 `rules/*.md`，跳过 `README.md` | 全局强制规则 |
| project rules | 确定 `project-key` 后读取 `rules/projects/<project-key>/**/*.md` | 项目强制规则；未确定项目时不得读取所有项目规则正文 |
| personal rules | 读取 `rules/user/**/*.md` | 使用者本地强制规则；不得覆盖 core/project rules |

Rules 的每条适用规则都必须写入 `process/context-pack.json` 的“适用强制规则”结构。若 rules 与需求、设计方案或已评审测试分析方案冲突，默认遵守 rules，并在“Rules 与输入冲突记录”中说明被覆盖的输入依据、覆盖原因和后续应用阶段。只有当前用户明确指令可以覆盖 rules。

Rules 内部冲突按 `core > project > personal` 处理；project/personal rules 只能细化更高层规则，不能放宽或违反更高层强制约束。
...
| L1 摘要命中 | 标题、适用范围、关键词、章节名和短摘要 | 判断是否需要摘录正文；对 rules 判断适用范围；对 project knowledge 文件判断强制应用环节 |
| L2 片段摘录 | 与当前需求直接相关的段落、表格行或章节摘要 | 写入 `context-pack.json`，供后续 skill 默认使用；Markdown 只作为渲染阅读版 |
...
`context-pack.json` 必须记录已检索来源、已注入片段、未注入但可能相关的来源和建议补读范围。后续 skill 如发现上下文不足，可以按这些来源进行受控补读；补读结果直接进入该 skill 的方法证据、风险备注、结构化过程记录或 review/coverage JSON，不要求刷新 context pack。
...
| 术语表/领域词表 | 文件名或标题包含 `glossary`、`term`、`术语`、`词表` | `input-fact-modeling`、`test-analysis-solution-generation` | 统一业务术语解释，不作为业务事实覆盖需求 |

无法自理解识别用途的 project knowledge 文件必须记录为 `unclassified`，写入后续补读建议；除非用户或文件 frontmatter 明确指定适用环节，否则不强制绑定到生成或审查环节，避免把未知资料硬套进流程。

context pack 必须在 JSON 中输出“项目知识阶段绑定”。后续被绑定的 skill 在开始本阶段活动前必须读取对应文件的相关章节，输出应用状态，并在方法证据、结构化过程记录或 coverage JSON 中记录来源。应用状态只能是：
...
- rules 如果适用范围命中当前 run，必须纳入 context pack；如果适用范围无法判断，记录为 `needs_scope_confirmation`，不得静默应用。
...
创建或刷新 `${PROJECT_ROOT}/outputs/runs/<run-id>/process/context-pack.json`，并由 `bin/render-run-markdown.py` 渲染 `process/context-pack.md`。JSON 内容包含：
...
- 未确定 `project-key` 时，不得读取 `rules/projects/*` 正文，避免跨项目强制规则污染。
- 不把 rules 当作普通 knowledge；适用 rules 必须进入 context pack 并在后续阶段应用或解释。
...
- 不把 `context-pack.json` 或其派生 `context-pack.md` 作为“最新全局上下文”复用给后续任务；后续任务必须重新按需求筛选或显式复用当前 run。
- 未经用户明确确认，不更新 memory 源文件。
- 本 skill 不直接向用户提问；只向 `clarification-gate` 提供候选。
- 后续 skill 如果发现 context pack 不足，可以读取 context pack 中列出的对应来源文件或当前需求明确指向的 project/personal 文件；读取时必须限定到相关标题、章节、关键词或表格片段，并在方法证据、结构化过程记录或 review/coverage JSON 中记录来源，不得自行全目录搜索或全量复制大文件。
- `process/context-pack.json` 是当前 run 的运行产物，不写入 `memory/`，也不作为长期 memory 源文件；同名 Markdown 只是派生阅读版。
