...
7. 如果审查测试分析方案，执行 `test-analysis-solution-check.md`，重点检查 `测试场景 -> 测试点 -> 测试点明细` 层级、场景级 E2E 测试点、非成功测试点明细第四层、预期结果兜底、TDI 泄漏和非完整用例化约束。
8. 如果审查测试设计方案，执行 `test-design-solution-check.md`，重点检查普通分支 `测试场景 -> 测试点 -> 测试点明细 -> 测试设计项`、非成功分支 `测试场景 -> 测试点 -> 测试点明细 -> 失败类型明细 -> 测试设计项`、TDI 表头、预期结果兜底和非完整用例化约束。
...
- 测试分析方案中的测试点和测试点明细是否保持非用例化，是否每个场景包含 `E2E场景测试`，是否非成功测试点明细拆出失败类型明细，且没有提前输出 TDI 或具体代表性数据组合。
- 测试设计方案中的测试设计项是否挂在普通测试点明细或失败类型明细这些叶子分析节点下，是否只表达代表性条件、数据、状态或组合，且没有进入完整用例步骤。
- 主交付件是否未使用 `**文本**`、`__文本__` 等 Markdown 加粗语法。
