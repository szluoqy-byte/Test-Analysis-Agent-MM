...
- 必选测试技术是否落到测试分析方案中的场景、测试点或测试点明细，或是否有方法证据解释。
- 必选测试技术是否有方法分析证据，或是否有不适用/输入不足解释。
...
- `预期结果` 是否有需求/设计依据；依据不足时是否降级为输入可支撑的保守预期，且没有补写未说明具体值。
- 主交付件是否没有 `未明确规则` 章节。
- 需求依据是否过于宽泛。
- 方法证据是否使用 `ME-*` 格式并关联 `TP-*` 或输入不足说明。
- 质量门禁是否存在失败项。
...
- 必选测试技术没有场景、测试点或测试点明细承接，且没有方法证据解释。
- 必选测试技术没有方法证据，且没有不适用/输入不足解释。
...
- 需求依据只写“需求文档”等宽泛来源。
- 测试点类型和测试技术明显不匹配。
- 大量测试点明细使用宽泛保守预期，说明需求或设计方案判定依据不足。
...
| 维度 | 2 分要求 |
|---|---|
| 需求理解 | 模块、角色、规则、状态、接口、数据对象和非范围均有体现；输入不足处只保留可支撑结论 |
| 测试技术应用 | 测试技术路由表完整，必选技术有 `ME-*` 证据或不适用/输入不足解释 |
...
- 任一维度不得为 `0`。
- 总分至少 `10/12`。
- 低于通过线时，如果原因是输出质量不足，必须修正；如果原因是需求缺失，必须保留输入不足说明。
...
- 因需求缺失导致无法满分时，最终报告必须保留刷新后的输入不足说明。
