...
| 失败类型充分性 | 非成功测试点明细下的失败类型能按业务逻辑、系统交互、外部依赖、异常返回、数据校验、权限控制、状态限制、接口契约或数据一致性等来源充分拆分 |
...
5. 检查非成功测试点明细下的失败类型拆分是否充分；如果多个失败来源被混在一个失败类型明细中，要求拆分或记录依据不足。
