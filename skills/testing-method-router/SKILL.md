...
本 skill 路由的是本项目内部专项方法参考和其对应测试技术，不提前选择测试点明细展开结果，不建议明细数量，也不决定测试步骤或预期结果。
...
- 被标记为 `必选` 的测试技术，最终必须生成测试点或方法证据承接。
- 被标记为 `必选` 的测试技术可以由方法证据、场景测试点或接口契约分析候选承接；不得把覆盖缺口留给测试分析方案生成、评审或细化环节重新分析原始需求来补洞。
