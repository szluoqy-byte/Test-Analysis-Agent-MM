...
description: 当用户提供设计方案文档时使用。负责从设计方案中提取接口、字段、状态、权限、数据依赖、配置、异常处理、非功能约束和设计缺口，形成可供测试技术路由、测试点生成和标题项生成消费的设计事实摘要。
---
...
- 本 skill 不生成测试点、标题项、测试步骤、完整预期结果或自动化脚本。
...
1. 识别设计方案来源、适用模块、关联需求片段和非范围说明。
2. 提取架构决策和关键流程，只保留会影响测试范围、风险、观察点或数据依赖的内容。
3. 提取接口契约，包括接口/集成点、请求字段、响应字段、错误码、鉴权、幂等、超时、重试、回调和兼容约束。
4. 提取字段和数据约束，包括必填、可空、格式、长度、枚举、范围、唯一性、精度和默认值。
5. 提取状态与生命周期，包括状态集合、合法迁移、终态、非法迁移、超时、回退、补偿和重复触发处理。
6. 提取权限与数据范围，包括角色、资源、动作、租户、归属、审批权限和可见范围。
7. 提取数据依赖，包括主数据、派生数据、缓存、统计、搜索、日志、消息、外部系统和一致性时机。
8. 提取配置与非功能约束，包括开关、灰度、降级、性能指标、容量限制、安全、可靠性和可观测性。
9. 对需求与设计冲突、设计缺失、来源不明或无法判断测试影响的内容登记 `CQ-*` 待确认候选。
10. 按 `templates/design-facts-template.md` 输出设计事实摘要，交给测试技术路由、测试点生成和标题项生成使用。
...
- 不把内部实现细节写入主交付件，除非它会影响测试输入条件、数据依赖、判定关注或待确认信息。
