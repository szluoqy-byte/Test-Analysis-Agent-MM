...
- 不把未确认业务规则写进 `knowledge/`；未确认事实只作为输入不足风险在 review/coverage findings 中表达，不建立单独确认流程。

## 生成测试分析方案时

- 需求文档是覆盖主账本，回答测试范围、业务目标和 what to test。
- 设计方案是落地依据，补充接口、字段、状态、数据依赖、异常处理、配置和非功能指标。
- 需求与设计冲突时，在输入事实模型或 review findings 中记录冲突，不静默二选一。
...
- 如果需求和设计方案没有说明错误提示、状态变化、错误码、接口返回或数据记录变化，预期结果只写输入可支撑的保守预期，不补写具体值。
