...
动态 project/personal knowledge 和 memory 来源由 `context-source-indexing` 写入 `process/context-pack.json`。强制规则由 `process/rules-pack.json` 独立承载，不作为普通动态来源处理。后续阶段只读取对本阶段可见的来源正文。
...
- 动态来源不得覆盖 `process/rules-pack.json` 中适用 rules、当前用户明确指令或输入文档事实。
