...
def collect_table_any(lines: list[str], headers: set[str]) -> list[tuple[int, list[str]]]:
    for header in headers:
        rows = collect_table(lines, header)
        if rows:
            return rows
    return []
...
def main() -> int:
    ...
    detail_rows = collect_table(lines, DETAIL_SUMMARY_HEADER)
    evidence_rows = collect_table_any(lines, EVIDENCE_HEADERS)
    gate_rows = collect_table(lines, GATE_HEADER)
    score_rows = collect_table(lines, SCORE_HEADER)

    if not testpoint_rows and not detail_rows:
    ...
    has_evidence_section = "方法分析证据摘要" in text or "方法证据摘要" in text
    ...
    for line_number, cells in detail_rows:
        if len(cells) != 4:
            errors.append(f"第 {line_number} 行：测试点明细摘要表列数异常")
            continue
        ...
        if not detail:
            errors.append(f"第 {line_number} 行：测试点明细为空")
        if not expected_result:
            errors.append(f"第 {line_number} 行：预期结果为空")
        previous_line = seen_details.get(detail)
        if previous_line:
            warnings.append(f"第 {line_number} 行：测试点明细与第 {previous_line} 行重复")
        seen_details[detail] = line_number
