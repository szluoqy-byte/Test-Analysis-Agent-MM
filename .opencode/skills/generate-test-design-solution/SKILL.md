...
| `test-design-solution-generation` | `TDI-*` 测试设计项、设计级预期结果、项目知识应用状态 | 确定性校验 |
...
- 每个普通测试点明细或失败类型明细下至少有 1 个测试设计项；测试设计项必须使用列表节点：`- TDI-001 <条件/数据/状态/组合>`，下一层缩进写 `- 预期结果：<明确预期结果或待人工分析确认>`。
- 主输出不得使用测试设计项表格；`TDI-*` 以列表节点挂在对应普通测试点明细或失败类型明细下，便于 Markdown 转脑图时保持层级。
- `TDI-*` 后的正文只写代表性条件、数据、状态或组合，例如“订单 ID 总长度等于 13 位”“订单已支付状态下重复提交取消请求”。
