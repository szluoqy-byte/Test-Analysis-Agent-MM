...
- 本方法参考 只产出风险方法参考记录、风险登记和级别建议，供 `test-analysis-solution-generation` 给测试点定级和补充风险备注。
...
|---|---|
| 需求明确说明 | 可直接作为测试点依据 |
| 经确认的项目历史缺陷或风险画像 | 可作为风险加权依据，但需保留来源 |
...
- 最终主交付件中只保留可追踪到需求、经确认的项目知识或风险确认点的风险备注；不把通用风险推断写成已确认业务规则。
- 如果可见动态来源中的 project/personal 风险画像不足，只能按 `sources[]` 记录的可见来源或当前需求明确指向的文件补读相关章节，并在方法参考记录中记录来源；不得全目录搜索或全量复制大文件。
