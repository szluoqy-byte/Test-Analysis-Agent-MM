...
本 skill 路由的是本项目内部专项分析 skill 和其对应测试技术，不提前选择测试设计项展开结果，不建议测试设计项数量，也不决定测试步骤或预期结果。
...
- 测试设计项阶段如何应用测试技术由 `test-design-solution-generation` 统一选择，本 skill 不提前映射或替代。

## 约束

- 跳过某个明显相关的测试技术时，必须给出原因。
- 被标记为 `必选` 的测试技术，最终必须生成测试点或待确认问题。
- 被标记为 `必选` 的测试技术可以由方法证据、场景测试点、接口测试点或待确认问题承接；不得把覆盖缺口留给测试设计方案生成、评审或细化环节重新分析原始需求来补洞。
