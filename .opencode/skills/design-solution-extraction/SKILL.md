...
description: 当用户提供设计方案文档时使用。负责从设计方案中提取接口、字段、状态、权限、数据依赖、配置、异常处理、非功能约束和设计缺口，形成可供测试技术路由、测试点生成和测试分析方案生成消费的设计事实摘要。
---
...
- 本 skill 不生成测试点、测试点明细、TDI、测试步骤、完整预期结果或自动化脚本。
...
12. 按 `templates/design-facts-template.md` 输出设计事实摘要，交给测试技术路由、测试点生成和测试分析方案生成使用。
...
- 不把内部实现细节写入主交付件，除非它会影响测试点、测试点明细或预期结果依据。
