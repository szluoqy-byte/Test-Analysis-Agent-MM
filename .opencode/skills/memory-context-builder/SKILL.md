...
Rules 的每条适用规则都必须写入 `process/context-pack.md` 的“适用强制规则”表。若 rules 与需求、设计方案或已评审测试分析方案冲突，默认遵守 rules，并在“Rules 与输入冲突记录”中说明被覆盖的输入依据、覆盖原因和后续应用阶段。只有当前用户明确指令可以覆盖 rules。
