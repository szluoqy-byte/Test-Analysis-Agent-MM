...
4. 检查每个 `TDI-*` 是否停留在测试设计项粒度：表达代表性条件、数据、状态或组合，不写步骤、脚本、接口调用过程或完整用例。
5. 检查设计项代表性是否充分；如果一个叶子分析节点明显需要有效、无效、边界、状态、权限或组合覆盖但设计项过少，提出补充建议。
6. 检查每个设计项的 `预期结果` 是否有可追溯依据；无法溯源的具体错误码、状态变化、提示文案、接口字段或阈值必须改为 `待人工分析确认`。
7. 按本阶段绑定的 project review knowledge 检查项目级设计漏覆盖；无法直接判断的检查项记录为 `insufficient_evidence`，不得编造成已确认缺陷。
8. 记录本阶段 project knowledge 应用状态，输出语义评审结论和逐项修正建议。
