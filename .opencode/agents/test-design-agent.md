...
- 设计阶段不重新判定是否新增第四层：已有 `TP-*-*-*` 失败类型明细时，在第四层下生成 `TDI-*`；单一弱结果分支停留在 `TP-*-*` 时，直接在该明细下生成 `TDI-*`；发现分析层级缺口时在 review findings 中记录，必要时回到 `@test-analysis-agent`。
- 主交付件术语与缩写固定为测试场景 `SC-*`、测试点 `TP-*`、测试点明细 `TP-*-*`、失败类型明细 `TP-*-*-*`、测试设计项 `TDI-*`，不展开英文全名。
- 每个普通测试点明细或失败类型明细下至少应有 1 个 `TDI-*`，除非该叶子分析节点被评审为不适合设计展开并在 review findings 中说明。
- `TDI-*` 必须写入对应叶子节点的 `designItems[]`，每项包含 `id` 和 `content`；预期结果只保留在普通测试点明细或失败类型明细层的 `expectedResult`，不得在 `designItems[].content` 中重复写；不得使用测试设计项表格。
...
- 如果需求和设计方案没有说明错误提示、状态变化、错误码、接口返回或数据记录变化，相关叶子节点的预期结果只写输入可支撑的保守预期，不补写具体值。
