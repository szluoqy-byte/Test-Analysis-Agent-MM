...
| 文件 | 定义 | 使用方式 |
|---|---|---|
| `${PROJECT_ROOT}/outputs/runs/<run-id>/process/context-pack.json` | 本次运行可见的 project/personal 动态来源索引 | 当前 run 内机器交互和追溯 |
| `${PROJECT_ROOT}/outputs/runs/<run-id>/process/context-pack.md` | 由 `context-pack.json` 渲染的人读版 | 只用于人工查看，不作为后续 skill 的事实源 |

## 使用流程

1. `context-source-indexing` 先根据用户参数、输入 frontmatter 和项目目录元信息确定 `project-key`。
2. 如果能唯一确定 `project-key`，索引 `projects/<project-key>/**/*.md` 的 frontmatter，跳过 `README.md`。
3. 索引 `user/**/*.md` 的 frontmatter，记录 personal 偏好或本地检查关注点来源。
4. 生成 `${PROJECT_ROOT}/outputs/runs/<run-id>/process/context-pack.json`，再按需渲染同名 Markdown。
5. 后续需求分析、测试技术路由、测试分析方案生成、测试设计方案生成和覆盖审查只读取 `sources[]` 中对当前阶段可见的 project/personal 文件正文。
...
- 大文件不需要维护 `index.md`；context pack JSON 只记录来源路径、名称、描述和阶段可见性，后续阶段按需补读相关章节。
