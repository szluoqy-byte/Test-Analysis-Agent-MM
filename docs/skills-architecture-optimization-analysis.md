...
| 路由层 | `testing-method-router` | 根据分析维度和触发信号选择测试技术和专项分析 skill | 测试技术路由表、技术范围待确认候选 |
| 专项分析层 | 9 个专项 skill | 产出 `ME-*` 方法证据和测试点候选 | 方法证据、测试点候选、技术缺口候选 |
...
| 路由层 | 保持独立 | 明确输出只到测试技术路由，不提前选择标题项展开结果 |
...
| 质量门禁 | 覆盖、追踪、测试技术应用、风险级别 | 失败则修正或登记待确认 |
...
当前 skills 架构已经具备独立 Agent 的基本闭环：入口清楚、专项测试技术齐全、生成链路完整、质量门禁和脚本可运行。

最值得优先优化的是两个方向：

1. 标准化契约：补齐核心 skill 的职责边界，并统一专项分析输出骨架。
2. 强化设计事实使用：让 `design-solution-extraction` 的结构化结果稳定进入测试技术路由、测试点生成和标题项生成。
