...
本 Agent 面向需求和设计方案生成标题大纲，不读取代码结构。因此当前测试技术库以黑盒/规格技术和经验技术为主，不覆盖 white-box 技术是合理的。

## 3. 当前技术对齐结果

| 当前技术文件 | ISTQB 对齐 | 判断 | 说明 |
|---|---|---|---|
| `specification-based/equivalence-boundary.md` | 等价类划分、边界值分析 | 直接匹配 | 当前把 EP 和 BVA 合并在一个技术文件中，符合标题项生成的使用方式 |
...
| `specification-based/scenario-usecase-userstory.md` | 场景/用例思路，偏业务流程测试 | 广义匹配 | 更像需求表达和业务流程覆盖方法，适合保留为标题大纲场景生成技术 |
...
| 质量属性技术 | `performance-efficiency.md`、`reliability-recoverability.md` | 都是非功能质量属性标题项生成；可共享指标、负载、故障和恢复表达 | 当非功能技术继续增加时 |
...
| `risk-based-testing.md` | 不是单一测试设计技术，但负责等级、深度和风险补充，是标题项大纲不可缺的策略层 |
| `error-guessing-checklist.md` | ISTQB 经验类技术直接匹配，且已承载通用缺陷模式 |

## 6. 当前结论

当前测试技术库不是纯 ISTQB 技术清单，而是“ISTQB 基础技术 + 工程化标题项生成技术”的组合。这个方向与 Agent 目标一致。

短期建议：

- 保持当前技术文件数量不变。
- 已直接匹配 ISTQB 的技术继续作为核心技术。
- 工程扩展技术继续保留，但在 README 中明确它们是标题项生成扩展，不是 ISTQB CTFL 核心技术。
