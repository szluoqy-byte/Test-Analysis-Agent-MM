...
本文审视当前 `knowledge/testcase-design-patterns/` 是否与 ISTQB 测试技术分类一致，并识别哪些模式是标准方法直接映射、哪些是工程扩展、哪些未来可以合并。
...
本 Agent 面向需求和设计方案生成标题大纲，不读取代码结构。因此当前模式库以黑盒/规格技术和经验技术为主，不覆盖 white-box 技术是合理的。

## 3. 当前模式对齐结果

| 当前模式 | ISTQB 对齐 | 判断 | 说明 |
|---|---|---|---|
| `specification-based/equivalence-boundary.md` | 等价类划分、边界值分析 | 直接匹配 | 当前把 EP 和 BVA 合并在一个模式中，符合标题项生成的使用方式 |
...
| `specification-based/scenario-usecase-userstory.md` | 场景/用例思路，偏业务流程测试 | 广义匹配 | 更像需求表达和业务流程覆盖方法，适合保留为标题大纲场景生成模式 |
| `risk-based/risk-based-test-design.md` | 风险驱动测试策略 | 策略匹配，不是设计技术 | ISTQB 体系重视风险驱动，但它更偏测试策略、优先级和深度控制 |
| `specification-based/data-combination.md` | 组合测试、pairwise、正交思路 | 工程扩展 | CTFL v4.0.1 常见黑盒技术不把组合测试列为核心项；但工程上很实用 |
| `specification-based/interface-contract.md` | API/契约测试 | 工程扩展 | 不是 CTFL 核心设计技术，更像面向接口和集成场景的领域模式 |
| `specification-based/processing-cycle.md` | 周期/批处理/时间窗口建模 | 工程扩展 | 可被状态转换、场景流和可靠性覆盖，当前是业务域便利模式 |
...
| 合并方向 | 可合并模式 | 原因 | 建议时机 |
|---|---|---|---|
| 规则判定模式 | `decision-point.md`、`decision-table.md`、`cause-effect.md` | 三者都处理条件、判定、动作和结果；ISTQB 直接核心是判定表 | 当维护者希望减少规格类模式数量时 |
| 状态周期模式 | `state-transition.md`、`processing-cycle.md` | 周期、批处理、重试周期常可建模为状态、事件和时间窗口 | 当状态/周期章节重复增多时 |
| 质量属性模式 | `performance-efficiency.md`、`reliability-recoverability.md` | 都是非功能质量属性标题项生成；可共享指标、负载、故障和恢复表达 | 当非功能模式继续增加时 |
| 场景流扩展 | `scenario-usecase-userstory.md` 与部分 `processing-cycle.md` | 批处理、定时任务、端到端业务流都依赖场景入口、触发和结束状态 | 当场景模式需要覆盖系统触发类流程时 |

## 5. 不建议合并的内容

| 模式 | 原因 |
|---|---|
| `equivalence-boundary.md` | ISTQB 直接核心技术，且标题项派生粒度稳定 |
| `state-transition.md` | ISTQB 直接核心技术，生命周期测试高频且独立 |
| `interface-contract.md` | 虽非 ISTQB CTFL 核心技术，但本 Agent 明确需要基于设计方案补接口、字段、错误码和幂等 |
| `data-combination.md` | 虽是工程扩展，但配置、渠道、版本、角色组合在标题大纲中高频出现 |
| `risk-based-test-design.md` | 不是单一设计技术，但负责等级、深度和风险补充，是标题项大纲不可缺的策略层 |
| `error-guessing-checklist.md` | ISTQB 经验类技术直接匹配，且已承载通用缺陷模式 |

## 6. 当前结论

当前模式库不是纯 ISTQB 技术清单，而是“ISTQB 基础技术 + 工程化标题项生成模式”的组合。这个方向与 Agent 目标一致。

短期建议：

- 保持当前模式数量不变。
- 已直接匹配 ISTQB 的模式继续作为核心模式。
- 工程扩展模式继续保留，但在 README 中明确它们是标题项生成扩展，不是 ISTQB CTFL 核心技术。
