...
每个测试场景下必须包含一个 `E2E场景测试` 测试点。`E2E场景测试` 是独立同级测试点，只维护 1 个端到端主流程成功闭环测试点明细；其他业务规则、异常处理、接口契约、权限、状态、回滚或补偿分支必须拆为同级 `TP-*`。当需求、设计方案或用户任务明确要求接口测试/API 契约覆盖时，接口测试或集成覆盖场景的非 E2E `TP-*` 必须先按接口、端点、消息、回调或集成点组织，再拆契约维度；通用接口规则必须在 `TP-*` 标题中说明适用范围。只有 `TP-*-*` 是非成功测试点明细时才新增 `TP-*-*-*` 第四层；`TP-*` 本身仍表示测试点主题。
