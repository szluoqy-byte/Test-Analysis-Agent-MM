...
17. coverage-review 通过且返工闭环完成后，运行 `python bin/build-final-report.py outputs/runs/<run-id> --scope design` 生成 `reports/design-final-report.json` 骨架，再使用 `final-report-generation` 填写 `coverageTree[]`、`coverageStatus` 和 `coverageReason`；填写后再次运行同一脚本重新计算 `summary` 并渲染 `reports/design-final-report.md`。最终报告只供人工审阅，不输出 `coverageGaps[]`，不触发返工。
