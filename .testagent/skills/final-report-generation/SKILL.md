...
4. 逐条 FACT 对照最终交付件，填写 `coverageTree[]`：
   - `coverageTree[].leafScenarioId`：只填写叶子 `SC-*`；SC 最多三层，完整父级路径由脚本从最终方案反查渲染。
   - `coverageTree[].testPoints[].testPointId`：填写该叶子 SC 下覆盖 FACT 的 `TP-*`。
   - `coverageTree[].testPoints[].testCases[]`：仅设计范围填写覆盖该 FACT 的 `TC-*`；分析范围保持空数组。
   - `coverageStatus`：只能取 `covered`、`partial`、`missing`、`not_applicable`。
   - `coverageReason`：仅 `partial`、`missing` 或 `not_applicable` 必填，解释为什么部分覆盖、缺失或不适用；`covered` 可留空。
