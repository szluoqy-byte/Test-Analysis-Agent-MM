...
2. `SC-*` 最多 3 层。非叶子场景只做业务路径分组，不挂测试点；只有叶子场景挂 `testPoints[]`。
3. 每个叶子场景必须包含一个 `E2E场景测试` 测试点。
4. `TP-*` 全局连续编号，不按场景重置。
5. `TP-*` 是验证目标，不是具体测试用例标题；它表达规则、路径、状态、权限、接口契约或风险。
6. 接口类场景下的非 E2E `TP-*` 应先定位接口、端点、消息、回调或集成点，再表达契约关注点。
