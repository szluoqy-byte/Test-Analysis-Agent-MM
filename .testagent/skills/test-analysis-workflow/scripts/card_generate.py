...
def setup_logger():
    ...
    logger = logging.getLogger('test_point_card_generate')
...
def generate_card(json_file_path: str, cida_info_file_path: str, spec_info: str) -> dict:
    logger.info("=" * 50)
    logger.info("开始生成测试点卡片")
    logger.info(f"测试点JSON文件路径: {json_file_path}")
    ...
    try:
        ...
        with open(json_file_path, "r", encoding="utf-8") as f:
            ...
            logger.info(f"文件读取成功, 测试点数量: {test_data.get('count', 0)}")
    except FileNotFoundError as e:
    ...
    data = {
        ...
        "subTitle": "测试点生成",
        "type": "local-iframe",
        "subTitleLink": f"/digital-test/agent/v1/static/{get_extension_name()}/webapps/testPoint/index.html",
        "state": "completed",
        "cardName": "test-point",
        "id": card_id,
        "cardId": card_id,
