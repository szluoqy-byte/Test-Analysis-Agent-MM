...
- 预期结果：<来自测试分析方案的预期结果；依据不足时保留输入可支撑的保守预期>
...
- 普通测试点明细或失败类型明细层保留一条 `- 预期结果：<明确预期结果，或输入可支撑的保守预期>`，继承测试分析方案的判定语义。
...
- `预期结果` 只能写需求、设计方案或上游测试分析方案明确支持的结果；如果需求和设计方案没有明确错误提示、状态变化、错误码、返回内容或记录变化，只写输入可支撑的保守预期，不补写具体值。
